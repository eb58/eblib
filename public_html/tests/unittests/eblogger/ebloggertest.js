...
test('testset: eblogger test message pattern', function () {
   ...
   ok( eblogger( { pattern: '<%=d%>' }).info('').indexOf('-'+ new Date().getDay())>=0 );
   ...
   equal( eblogger( { pattern: '<%=t%> <%=f%>: <%=m%>', func:'functionName' }).info('theMessage'), 'INFO  functionName: theMessage');
});
...
test('testset: eblogger test late evaluation', function () {
   ...
   x = false, eblogger().info(  () => { x=true; } ); ok( x === true,  'lamba called' ); 
   x = false, eblogger().debug( () => { x=true; } ); ok( x === false, 'lamba not called' ); 
